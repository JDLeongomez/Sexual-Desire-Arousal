...
dat <- read.csv("Data/BD_Heterosexuales_Vertical_BIG.csv") |>
  ...
  rename(Participant = Participante, Age = EdadParticipante, `Preferred sex` = PrefSex,
         Gender = Sexo, `Contraceptive uso` = Anticoncep, `Last period` = UltimoPer,
         `Period day` = Dia_ciclo, Education = Escolaridad, Location = Residencia,
         `Location (other)` = Residencia_3_TEXT, `Medical history` = AntMed,
         `Sexual orientation` = OS, `Relationship status` = EstRel,
         `Relationship duration` = TiempoRP, `Partner gender` = SexPareja,
         `Relationship type` = TipoRel, `Age at first intercourse` = Primera.ExpSex,
         ...
         Relationship = TieneRelacion, `MGH-SFQ (total)` = MGH.SFQ_Total,
         ...
         `Stimuli code` = Codigo_Estimulo, `Stimuli sex` = Sexo_Estimulo,
...
## Hypothesis 1: Sexual desire by gender, relationship type and sexual desire dimension {#hyp1}
...
## Hypothesis 2: Sexual arousal by gender, relationship type, stimuli sex and sexual desire {#hypothesis2}
...
### Hypothesis 2a: Interaction between gender, stimuli sex, relationship type and sexual desire dimension as predictors of subjective sexual arousal to erotic stimuli {#hypothesis2a}
...
##### Table \@ref(tab:tab-m2a). Regression-type table for the interaction between `Gender`, `Stimuli sex`, `Relationship type` and `Sexual desire dimension`
...
p2a.d <- ggplot(m2a.dat, aes(x = `Dyadic sexual desire (Attractive person) (C)`,
  ...
  geom_text(data = slop.m2a_d.tab.plot_f,
            ...
            label = paste("B = ", slop.m2a_d.tab.plot_f$Coefficient, 
                          ...
                          ifelse(is.na(slop.m2a_d.tab.plot_f$sig), "", 
                                 slop.m2a_d.tab.plot_f$sig)),
  ...
  geom_text(data = slop.m2a_d.tab.plot_m,
            ...
            label = paste("B = ", slop.m2a_d.tab.plot_m$Coefficient, 
                          ...
                          ifelse(is.na(slop.m2a_d.tab.plot_m$sig), "", 
                                 slop.m2a_d.tab.plot_m$sig)),
...
p2a.e <- ggplot(m2a.dat, aes(x = `Dyadic sexual desire (Partner) (C)`,
  ...
  geom_text(data = slop.m2a_e.tab.plot_sin,
            ...
            label = paste("B = ", slop.m2a_e.tab.plot_sin$Coefficient, 
                          ...
                          ifelse(is.na(slop.m2a_e.tab.plot_sin$sig), "", 
                                 slop.m2a_e.tab.plot_sin$sig)),
...
### Hypothesis 2b: Interaction between gender, stimuli sex,relationship type and sexual desire dimension as predictors of subjective sexual arousal to non-erotic stimuli {#hypothesis2b}
...
m2b <- lmer(`Subjective sexual arousal` ~
            ...
            Relationship*Gender*`Stimuli sex`*`Dyadic sexual desire (Attractive person) (C)` +
            Relationship*Gender*`Stimuli sex`*`Dyadic sexual desire (Partner) (C)` +
            (1 | `Stimuli code`) + (1 + `Stimuli sex` | Participant),
           data = dat.nero, control = lmerControl(optimizer = "bobyqa"))
```
...
check_model(m2b, check = c("linearity", "homogeneity", "qq", "reqq"))
```
...
##### Table \@ref(tab:tab-m2b). Regression-type table for the interaction between `Gender`, `Stimuli sex`, `Relationship type` and `Sexual desire dimension`
