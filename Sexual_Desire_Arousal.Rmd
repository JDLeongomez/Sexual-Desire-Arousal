...
anova_results <- bind_cols(
  bind_cols(
    ...
    unite(col = "df", DFn:DFd, sep = ", "), # Combine numerator and denominator df
  bind_cols(
    anova_summary(Anova(ctl_PD, type = 3)), # Type III ANOVA for Dyadic TSD Partner
    epsilon_squared(ctl_PD)
  ) |>
    unite(col = "df", DFn:DFd, sep = ", "),
  bind_cols(
    anova_summary(Anova(ctl_APD, type = 3)), # Type III ANOVA for Dyadic TSD Attractive Person
...
# Random effects: Intercepts per stimulus; intercepts & slopes for stimuli per participant
m_stim_cont <- lmer(
...
p_m3b_clmm <- emmeans(m3b_clmm, ~ Dyadic.TSD.Attractive.Person | Gender * Relationship,
  ...
  facet_wrap(~Gender, ncol = 1) + # Create separate plots for each gender
  labs(
    y = "Predicted Subjective Sexual Arousal", x = "Dyadic TSD Attractive Person",
    title = "CLMM"
  ) +
  theme_tq() + # Apply custom theme
  theme(legend.position = "bottom") +
