...
\hypersetup{
  pdftitle={Are there differences in the predictive relationship between trait sexual desire and subjective sexual arousal toward sexual stimuli dependent on gender and relationship status?},
...
\begin{figure}
\centering
\includegraphics{Deseo_excitacion_sexual_files/figure-latex/sample-plot-1.pdf}
...
Table of estimated marginal means and contrasts between genders. All estimated marginal means and contrasts were calculated using the \texttt{emmeans} function from the \texttt{emmeans} package (\protect\hyperlink{ref-emmeanscit}{Lenth, 2022}).
...
Table of estimated marginal means and contrasts between the three types of sexual desire dimension. All estimated marginal means and contrasts were calculated using the \texttt{emmeans} function from the \texttt{emmeans} package (\protect\hyperlink{ref-emmeanscit}{Lenth, 2022}).
...
Stimuli sex [Female] & 0.46 &  &  & 0.05 & 360.27 & 8.50 & \textbf{< 0.0001}\\
...
Relationship [Stable] × Stimuli sex [Female] & 0.02 &  &  & 0.05 & 304.00 & 0.45 & 0.65\\
Gender [Women] × Stimuli sex [Female] & -0.81 &  &  & 0.05 & 304.00 & -17.66 & \textbf{< 0.0001}\\
...
Stimuli sex [Female] × Solitary sexual desire (C) & -0.01 &  &  & 0.01 & 304.00 & -1.05 & 0.3\\
...
\NormalTok{                                             slop.m2a\_d.tab.plot\_f}\SpecialCharTok{$}\NormalTok{X97.}\FloatTok{5.}\NormalTok{, }\AttributeTok{sep =} \StringTok{", "}\NormalTok{), }
                          ...
                          \FunctionTok{ifelse}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(slop.m2a\_d.tab.plot\_f}\SpecialCharTok{$}\NormalTok{sig), }\StringTok{""}\NormalTok{, slop.m2a\_d.tab.plot\_f}\SpecialCharTok{$}\NormalTok{sig)),}
...
\NormalTok{                                             slop.m2a\_d.tab.plot\_m}\SpecialCharTok{$}\NormalTok{X97.}\FloatTok{5.}\NormalTok{, }\AttributeTok{sep =} \StringTok{", "}\NormalTok{), }
                          ...
                          \FunctionTok{ifelse}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(slop.m2a\_d.tab.plot\_m}\SpecialCharTok{$}\NormalTok{sig), }\StringTok{""}\NormalTok{, slop.m2a\_d.tab.plot\_m}\SpecialCharTok{$}\NormalTok{sig)),}
...
\NormalTok{                                             slop.m2a\_e.tab.plot\_sin}\SpecialCharTok{$}\NormalTok{X97.}\FloatTok{5.}\NormalTok{, }\AttributeTok{sep =} \StringTok{", "}\NormalTok{), }
                          ...
                          \FunctionTok{ifelse}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(slop.m2a\_e.tab.plot\_sin}\SpecialCharTok{$}\NormalTok{sig), }\StringTok{""}\NormalTok{, slop.m2a\_e.tab.plot\_sin}\SpecialCharTok{$}\NormalTok{sig)),}
...
Stimuli sex [Female] & 0.19 &  &  & 0.05 & 302.42 & 3.89 & \textbf{< 0.001}\\
...
Relationship [Stable] × Stimuli sex [Female] & 0.03 &  &  & 0.04 & 304.00 & 0.85 & 0.39\\
Gender [Women] × Stimuli sex [Female] & -0.49 &  &  & 0.04 & 304.00 & -12.14 & \textbf{< 0.0001}\\
...
Stimuli sex [Female] × Solitary sexual desire (C) & -0.01 &  &  & 0.00 & 304.00 & -2.03 & \textbf{0.0435}\\
...
Table of estimated slopes for Solitary sexual desire on Subjective sexual arousal for each gender from model 2b. Solitary sexual desire values were centered. Slopes were calculated using the \texttt{sim\_slopes} function from the \texttt{interactions} package (\protect\hyperlink{ref-interactionscit}{Long, 2019}).
...
Table of estimated slopes for Solitary sexual desire on Subjective sexual arousal for each stimuli sex from model 2b. Solitary sexual desire values were centered. Slopes were calculated using the \texttt{sim\_slopes} function from the \texttt{interactions} package (\protect\hyperlink{ref-interactionscit}{Long, 2019}).
...
\emph{pander(v.0.6.5)}, \emph{Hmisc(v.5.1-0)}, \emph{berryFunctions(v.1.22.0)}, \emph{rstatix(v.0.7.2)}, \emph{effectsize(v.0.8.3)}, \emph{scales(v.1.2.1)}, \emph{ggpmisc(v.0.5.2)}, \emph{ggpp(v.0.5.2)}, \emph{MetBrewer(v.0.2.0)}, \emph{psych(v.2.3.3)}, \emph{kableExtra(v.1.3.4)}, \emph{performance(v.0.10.3)}, \emph{emmeans(v.1.8.6)}, \emph{interactions(v.1.1.5)}, \emph{tidyquant(v.1.0.7)}, \emph{quantmod(v.0.4.22)}, \emph{TTR(v.0.24.3)}, \emph{PerformanceAnalytics(v.2.0.4)}, \emph{xts(v.0.13.1)}, \emph{zoo(v.1.8-12)}, \emph{ggpubr(v.0.6.0)}, \emph{lubridate(v.1.9.2)}, \emph{forcats(v.1.0.0)}, \emph{stringr(v.1.5.0)}, \emph{dplyr(v.1.1.2)}, \emph{purrr(v.1.0.1)}, \emph{readr(v.2.1.4)}, \emph{tidyr(v.1.3.0)}, \emph{tibble(v.3.2.1)}, \emph{ggplot2(v.3.4.2)}, \emph{tidyverse(v.2.0.0)}, \emph{lmerTest(v.3.1-3)}, \emph{lme4(v.1.1-33)}, \emph{Matrix(v.1.5-4.1)}, \emph{car(v.3.1-2)}, \emph{carData(v.3.0-5)}, \emph{ltm(v.1.2-0)}, \emph{polycor(v.0.8-1)}, \emph{msm(v.1.7)}, \emph{MASS(v.7.3-58.2)}, \emph{readxl(v.1.4.2)} and \emph{knitr(v.1.42)}

\textbf{loaded via a namespace (and not attached):}
\emph{TH.data(v.1.1-2)}, \emph{minqa(v.1.2.5)}, \emph{colorspace(v.2.1-0)}, \emph{ggsignif(v.0.6.4)}, \emph{htmlTable(v.2.4.1)}, \emph{estimability(v.1.4.1)}, \emph{base64enc(v.0.1-3)}, \emph{parameters(v.0.21.0)}, \emph{rstudioapi(v.0.14)}, \emph{farver(v.2.1.1)}, \emph{MatrixModels(v.0.5-1)}, \emph{fansi(v.1.0.4)}, \emph{mvtnorm(v.1.1-3)}, \emph{xml2(v.1.3.4)}, \emph{codetools(v.0.2-19)}, \emph{splines(v.4.2.3)}, \emph{mnormt(v.2.1.1)}, \emph{Formula(v.1.2-5)}, \emph{polynom(v.1.4-1)}, \emph{jsonlite(v.1.8.4)}, \emph{nloptr(v.2.0.3)}, \emph{broom(v.1.0.4)}, \emph{cluster(v.2.1.4)}, \emph{compiler(v.4.2.3)}, \emph{httr(v.1.4.6)}, \emph{backports(v.1.4.1)}, \emph{fastmap(v.1.1.1)}, \emph{cli(v.3.6.1)}, \emph{quantreg(v.5.95)}, \emph{admisc(v.0.32)}, \emph{htmltools(v.0.5.5)}, \emph{tools(v.4.2.3)}, \emph{coda(v.0.19-4)}, \emph{gtable(v.0.3.3)}, \emph{glue(v.1.6.2)}, \emph{Rcpp(v.1.0.10)}, \emph{cellranger(v.1.1.0)}, \emph{vctrs(v.0.6.2)}, \emph{svglite(v.2.1.1)}, \emph{nlme(v.3.1-162)}, \emph{insight(v.0.19.2)}, \emph{xfun(v.0.39)}, \emph{rvest(v.1.0.3)}, \emph{timechange(v.0.2.0)}, \emph{lifecycle(v.1.0.3)}, \emph{hms(v.1.1.3)}, \emph{parallel(v.4.2.3)}, \emph{sandwich(v.3.0-2)}, \emph{SparseM(v.1.81)}, \emph{expm(v.0.999-7)}, \emph{yaml(v.2.3.7)}, \emph{curl(v.5.0.0)}, \emph{see(v.0.7.5)}, \emph{gridExtra(v.2.3)}, \emph{rpart(v.4.1.19)}, \emph{stringi(v.1.7.12)}, \emph{highr(v.0.10)}, \emph{bayestestR(v.0.13.1)}, \emph{checkmate(v.2.2.0)}, \emph{boot(v.1.3-28.1)}, \emph{rlang(v.1.1.1)}, \emph{pkgconfig(v.2.0.3)}, \emph{systemfonts(v.1.0.4)}, \emph{evaluate(v.0.21)}, \emph{lattice(v.0.20-45)}, \emph{patchwork(v.1.1.2)}, \emph{htmlwidgets(v.1.6.2)}, \emph{labeling(v.0.4.2)}, \emph{cowplot(v.1.1.1)}, \emph{tidyselect(v.1.2.0)}, \emph{magrittr(v.2.0.3)}, \emph{bookdown(v.0.34)}, \emph{R6(v.2.5.1)}, \emph{generics(v.0.1.3)}, \emph{multcomp(v.1.4-23)}, \emph{mgcv(v.1.8-42)}, \emph{foreign(v.0.8-84)}, \emph{pillar(v.1.9.0)}, \emph{withr(v.2.5.0)}, \emph{jtools(v.2.2.1)}, \emph{nnet(v.7.3-18)}, \emph{datawizard(v.0.7.1)}, \emph{survival(v.3.5-3)}, \emph{abind(v.1.4-5)}, \emph{crayon(v.1.5.2)}, \emph{Quandl(v.2.11.0)}, \emph{utf8(v.1.2.3)}, \emph{tzdb(v.0.4.0)}, \emph{rmarkdown(v.2.21)}, \emph{grid(v.4.2.3)}, \emph{data.table(v.1.14.8)}, \emph{digest(v.0.6.31)}, \emph{webshot(v.0.5.4)}, \emph{xtable(v.1.8-4)}, \emph{numDeriv(v.2016.8-1.1)}, \emph{munsell(v.0.5.0)}, \emph{viridisLite(v.0.4.2)} and \emph{quadprog(v.1.5-8)}
