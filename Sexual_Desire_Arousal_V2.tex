...
\NormalTok{  aovTab }\OtherTok{\textless{}{-}} \FunctionTok{bind\_cols}\NormalTok{(}\FunctionTok{anova}\NormalTok{(model),}
    ...
    \FunctionTok{kable}\NormalTok{(}\AttributeTok{digits =} \DecValTok{2}\NormalTok{,}
          ...
          \AttributeTok{linesep =} \StringTok{""}\NormalTok{,}
          \AttributeTok{caption =}\NormalTok{ custom\_caption,}
          \AttributeTok{col.names =} \FunctionTok{c}\NormalTok{(}\StringTok{"Effect"}\NormalTok{, }\StringTok{"$df$"}\NormalTok{, }\StringTok{"$F$"}\NormalTok{, }\StringTok{"$p$"}\NormalTok{, }\StringTok{"$}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{omega\^{}2\_p$"}\NormalTok{),}
          \AttributeTok{escape =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{kable\_styling}\NormalTok{(}\AttributeTok{latex\_options =} \FunctionTok{c}\NormalTok{(}\StringTok{"HOLD\_position"}\NormalTok{, }\StringTok{"scale\_down"}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
...
\StringTok{                              As effect size, we report partial omega squared }
\StringTok{                              ($}\SpecialCharTok{\textbackslash{}\textbackslash{}\textbackslash{}\textbackslash{}}\StringTok{omega\^{}2\_p$; see}
\StringTok{                              }\SpecialCharTok{\textbackslash{}\textbackslash{}\textbackslash{}\textbackslash{}}\StringTok{cite\{albersWhenPowerAnalyses2018\}).}
\StringTok{                              Significant effects are in bold."}\NormalTok{),}
             \AttributeTok{escape =} \ConstantTok{FALSE}\NormalTok{,}
             \AttributeTok{threeparttable =} \ConstantTok{TRUE}\NormalTok{,}
...
Effect & $df$ & $F$ & $p$ & $\epsilon^2_p$\\
\midrule
Gender & 1, 316 & 15.49 & \textbf{< 0.001} & 0.04\\
...
\subsection{Data filtering for hypotheses 2 and 3}\label{data-filtering-for-hypotheses-2-and-3}
...
\NormalTok{             (}\DecValTok{1} \SpecialCharTok{+} \StringTok{\textasciigrave{}}\AttributeTok{Stimuli content}\StringTok{\textasciigrave{}} \SpecialCharTok{|}\NormalTok{ Participant),}
           \AttributeTok{data =}\NormalTok{ dat,}
           \AttributeTok{control =} \FunctionTok{lmerControl}\NormalTok{(}\AttributeTok{optimizer =} \StringTok{"bobyqa"}\NormalTok{))}

\FunctionTok{anova.sig.lmer}\NormalTok{(}\AttributeTok{model =}\NormalTok{ m\_stim\_cont, }\AttributeTok{custom\_caption =} \StringTok{"Effects of relationship type and gender on }
\StringTok{          Dyadic sexual desire: Partner"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{table}[H]
\centering
\caption{\label{tab:tab-m-stimuli-content}Effects of relationship type and gender on 
          Dyadic sexual desire: Partner}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{threeparttable}
\begin{tabular}[t]{lcccl}
\toprule
Effect & $df$ & $F$ & $p$ & $\omega^2_p$\\
\midrule
Gender & 1, 321 & 42.47 & \textbf{< 0.0001} & 0.11\\
Stimuli sex & 1, 136.32 & 341.36 & \textbf{< 0.0001} & 0.71\\
Stimuli content & 1, 354.57 & 87.53 & \textbf{< 0.0001} & 0.20\\
Gender × Stimuli sex & 1, 44434 & 20218.93 & \textbf{< 0.0001} & 0.31\\
Gender × Stimuli content & 1, 321 & 5.02 & \textbf{0.0257} & 0.01\\
Stimuli sex × Stimuli content & 1, 136.32 & 33.39 & \textbf{< 0.0001} & 0.19\\
Gender × Stimuli sex × Stimuli content & 1, 44434 & 1137.56 & \textbf{< 0.0001} & 0.02\\
\bottomrule
\end{tabular}
\begin{tablenotes}[para]
\item \textit{Note: } 
\item Results are type III ANOVA.
                              $R^2_{conditional}$ = 0.58, $R^2_{marginal}$ = 0.305. Gender = participants gender (women, men); 
...
\begin{verbatim}
## Warning in model.frame.default(formula, data = data, ...): variable '`Stimuli
## sex`' is not a factor
\end{verbatim}

\begin{verbatim}
## Warning in model.frame.default(formula, data = data, ...): variable '`Stimuli
## content`' is not a factor
\end{verbatim}
...
\midrule
\addlinespace[0.3em]
\multicolumn{12}{l}{\textbf{Gender: Women - Stimuli content: Erotic}}\\
\hspace{1em}Female & 1.46 & 0.08 & Inf & 1.31 & 1.61 & Female - Male & -0.77 & 0.05 & Inf & -14.18 & \textbf{< 0.0001}\\
\hspace{1em}Male & 2.23 & 0.08 & Inf & 2.08 & 2.38 &  &  &  &  &  & \\
\addlinespace[0.3em]
\hline
\multicolumn{12}{l}{\textbf{Gender: Women - Stimuli content: Non-erotic}}\\
\hspace{1em}Female & 1.12 & 0.07 & Inf & 0.99 & 1.25 & Female - Male & -0.57 & 0.06 & Inf & -9.03 & \textbf{< 0.0001}\\
\hspace{1em}Male & 1.69 & 0.07 & Inf & 1.56 & 1.82 &  &  &  &  &  & \\
\addlinespace[0.3em]
\hline
\multicolumn{12}{l}{\textbf{Gender: Men - Stimuli content: Erotic}}\\
\hspace{1em}Female & 3.84 & 0.09 & Inf & 3.67 & 4.01 & Female - Male & 2.75 & 0.06 & Inf & 49.66 & \textbf{< 0.0001}\\
\hspace{1em}Male & 1.09 & 0.09 & Inf & 0.92 & 1.26 &  &  &  &  &  & \\
\addlinespace[0.3em]
\multicolumn{12}{l}{\textbf{Gender: Men - Stimuli content: Non-erotic}}\\
\hspace{1em}Female & 2.65 & 0.07 & Inf & 2.51 & 2.79 & Female - Male & 1.60 & 0.06 & Inf & 25.08 & \textbf{< 0.0001}\\
\hspace{1em}Male & 1.05 & 0.07 & Inf & 0.90 & 1.19 &  &  &  &  &  & \\
\bottomrule
\end{tabular}
\begin{tablenotes}[para]
\item \textit{Note: } 
...
\begin{Highlighting}[]
\CommentTok{\# Gender main effect}
\FunctionTok{ggplot}\NormalTok{(dat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \StringTok{\textasciigrave{}}\AttributeTok{Stimuli sex}\StringTok{\textasciigrave{}}\NormalTok{, }\AttributeTok{y =} \StringTok{\textasciigrave{}}\AttributeTok{Subjective sexual arousal}\StringTok{\textasciigrave{}}\NormalTok{, }
  ...
  \FunctionTok{stat\_pvalue\_manual}\NormalTok{(t.stim\_cont, }
                     \AttributeTok{label =} \StringTok{"p.signif"}\NormalTok{, }
                     \AttributeTok{y.position =} \FunctionTok{c}\NormalTok{(}\FloatTok{2.7}\NormalTok{, }\DecValTok{3}\NormalTok{, }\FloatTok{4.1}\NormalTok{, }\DecValTok{3}\NormalTok{), }
