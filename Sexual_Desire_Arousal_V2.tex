...
To ensure that the three sexual desire dimensions were not influenced by Relationship Duration or MGSS sexual satisfaction (Partner), we standardized the scores using a two-step process.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Modeling the relationships: we performed linear regressions where each sexual desire dimension was predicted using Relationship Duration and MGSS sexual satisfaction (Partner). This allowed us to estimate their effects.
\item
  Removing the effects: we extracted the residuals from each regression model, which represent the variation in sexual desire that is independent of Relationship Duration and MGSS sexual satisfaction (Partner). These residuals were then standardized to ensure comparability across participants.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Select only participants in stable relationships}
\NormalTok{dat\_ctl }\OtherTok{\textless{}{-}}\NormalTok{ dat }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{group\_by}\NormalTok{(Participant) }\SpecialCharTok{|\textgreater{}}  \CommentTok{\# Group by participant}
  \FunctionTok{slice\_head}\NormalTok{() }\SpecialCharTok{|\textgreater{}}           \CommentTok{\# Select the first observation per participant}
  \FunctionTok{filter}\NormalTok{(Relationship }\SpecialCharTok{==} \StringTok{"Stable"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}  \CommentTok{\# Retain only stable relationships}
  \FunctionTok{ungroup}\NormalTok{()}

\CommentTok{\# Fit linear models to predict each dimension of sexual desire }
\CommentTok{\# based on relationship duration and partner sexual satisfaction}
...
\CommentTok{\# Prepare a dataset with relevant variables, removing missing values}
...
\subparagraph{Figure \ref{fig:assu-m1a}. Posterior predictive checks (PPCs)}\label{figure-reffigassu-m1a.-posterior-predictive-checks-ppcs}

PPCs were performed using the \texttt{check\_model} function from the \texttt{performance} package \autocite{ludecke2021}, and reported in Fig. \ref{fig:assu-m1a}.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggarrange}\NormalTok{(}\FunctionTok{plot}\NormalTok{(}\FunctionTok{check\_model}\NormalTok{(m1a\_prop, }
               ...
               \AttributeTok{colors =} \FunctionTok{c}\NormalTok{(}\StringTok{"red"}\NormalTok{, }\StringTok{"black"}\NormalTok{)) }\SpecialCharTok{+}
            \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Original (proportion) Solitary TSD"}\NormalTok{,}
                 \AttributeTok{subtitle =} \ConstantTok{NULL}\NormalTok{) }\SpecialCharTok{+}
            \FunctionTok{theme\_tq}\NormalTok{(),}
          \FunctionTok{plot}\NormalTok{(}\FunctionTok{check\_model}\NormalTok{(m1a\_norm, }
                           \AttributeTok{panel =} \ConstantTok{FALSE}\NormalTok{,}
                           \AttributeTok{check =} \StringTok{"pp\_check"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{PP\_CHECK,}
               \AttributeTok{colors =} \FunctionTok{c}\NormalTok{(}\StringTok{"red"}\NormalTok{, }\StringTok{"black"}\NormalTok{)) }\SpecialCharTok{+}
            \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Transformed (normalized) Solitary TSD"}\NormalTok{,}
                 \AttributeTok{subtitle =} \ConstantTok{NULL}\NormalTok{) }\SpecialCharTok{+}
            \FunctionTok{theme\_tq}\NormalTok{(),}
...
\begin{figure}
\centering
\includegraphics{Sexual_Desire_Arousal_V2_files/figure-latex/assu-m1a-1.pdf}
\caption{\label{fig:assu-m1a}Posterior predictive check. \textbf{(a)} Original (proportion) Solitary TSD; \textbf{(b)} Transformed (normalized) Solitary TSD. In both panels, red lines represent the observed data, and thin black lines represent 50 iterations of simulated data from each model. In this case, simulated data from the normalized Solitary TSD model (panel b) are more similar to the observed outcome, so this model is preferred.}
\end{figure}
