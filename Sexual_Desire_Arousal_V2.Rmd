...
anova.sig.lmer <- function(model, custom_caption) {
  aovTab <- bind_cols(anova(model),
    ...
    kable(digits = 2,
          ...
          linesep = "",
          caption = custom_caption,
          col.names = c("Effect", "$df$", "$F$", "$p$", "$\\omega^2_p$"),
          escape = FALSE) |>
    kable_styling(latex_options = c("HOLD_position", "scale_down")) |>
    footnote(general = paste0("Results are type III ANOVA.
                              ...
                              As effect size, we report partial omega squared 
                              ($\\\\omega^2_p$; see
                              \\\\cite{albersWhenPowerAnalyses2018}).
                              Significant effects are in bold."),
             escape = FALSE,
...
#### Table \@ref(tab:tab-m1a). Regression-type table for the interaction between `Relationship type`, and `Gender`
...
#### Figure \@ref(fig:fig-h1a). Differences among the three dimensions of sexual desire
...
### Hypothesis 1b: Solitary TSD {#hypothesis1b}
...
#### Table \@ref(tab:tab-m1b). Regression-type table for the interaction between `Relationship type`, and `Gender`
...
#### Figure \@ref(fig:fig-h1b). Differences among the three dimensions of sexual desire
...
### Hypothesis 1c: Solitary TSD {#hypothesis1c}
...
#### Table \@ref(tab:tab-m1c). Regression-type table for the interaction between `Relationship type`, and `Gender`
...
#### Figure \@ref(fig:fig-h1c). Differences among the three dimensions of sexual desire
...
## Data filtering for hypotheses 2 and 3

To avoid over-complicating the model, first we tested whether the effects on sexual arousal were stronger depending on the content of the stimuli (erotic versus non-erotic). This was, in fact, the case:

```{r tab-m-stimuli-content, message = FALSE}
...
anova.sig.lmer(model = m_stim_cont, custom_caption = "Effects of relationship type and gender on 
          Dyadic sexual desire: Partner")
```

```{r tab-stim-cont-emms, message = FALSE}
...
```{r fig-m-stimuli-content, results = "hold", fig.height = 4, fig.width = 8, fig.cap = "Effects of stimuli content (erotic, non-erotic) on subjective sexual arousal for women's (left panel) and men's (right panel) scores of male and female stimuli (see Table \\@ref(tab:tab-m-stimuli-content); for detailed results, see Table \\@ref(tab:tab-stim-cont-emms)). Dots and bars represent estimated marginal means and 95% CI. In all cases, significant effects are represented with lines and stars: \\**p* < 0.05, \\*\\**p* < 0.01, \\*\\*\\**p* < 0.001, \\*\\*\\*\\**p* < 0.0001.", warning = FALSE, message = FALSE}
# Gender main effect
ggplot(dat, aes(x = `Stimuli sex`, y = `Subjective sexual arousal`, 
...
Considering this, we tested all predictions of hypotheses 2 and 3 only on responses to erotic stimuli.
...
We fitted models with both the original (proportion; `m1a_prop`) and transformed (normalized; `m1a_norm`) TSD scores, and performed posterior predictive checks (PPCs) to select the best model. As shown elsewhere [e.g., @gabryVisualizationBayesianWorkflow2019], if simulated data from one model are more similar to the observed outcome, that model is likely to be preferred.
...
```{r ppc-m2a, message = FALSE, fig.height = 4, fig.cap = "Posterior predictive check. **(a)** Original (proportion) Solitary TSD; **(b)** Transformed (normalized) Solitary TSD. In both panels, red lines represent the observed data, and thin black lines represent 50 iterations of simulated data from each model. "}
ppc_m2a <- ggarrange(plot(check_model(m1a_prop, 
                                      panel = FALSE,
                                      check = "pp_check")$PP_CHECK,
                          colors = c("red", "grey30")) +
                       labs(title = NULL, subtitle = NULL) +
